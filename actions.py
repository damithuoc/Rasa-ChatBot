...
class ActionMeetup(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        print("===========================MEETUP_KEY==========================", os.environ['MEETUP_KEY'])

        r = requests.get('https://api.meetup.com/find/groups?key={}&location={}&text={}&upcoming_events=true'.format(
...
class ActionJoinMeetup(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        r = requests.post(
            'https://api.meetup.com/2/profile?key={}&group_urlname={}'.format(os.environ['MEETUP_KEY'], meetup_id))
        response = json.loads(r.text)
...
class ActionJoinEvent(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        r = requests.post(
            'https://api.meetup.com/2/rsvp?key={}&event_id={}&rsvp=yes'.format(os.environ['MEETUP_KEY'], next_event_id))
        response = json.loads(r.text)
        ...
        try:
            ...
            event_time = datetime.datetime.fromtimestamp(int(response['event']['time']) / 1000).strftime(
                '%Y-%m-%d %H:%M:%S')

            dispatcher.utter_message(
                "All done. I have booked you a spot at the event {} which is happening at {}".format(event_name,
                                                                                                     event_time))
